...
impl Menu {
    pub fn render(
        ...
        match &self.state {
            MenuState::ModeSelect { modes, selected_index } => {
                self.render_mode_select(frame, inner, config, modes, *selected_index);
            }
            MenuState::SourceSelect { mode_name, available_sources, selected_index, .. } => {
                self.render_source_select(frame, inner, config, mode_name, available_sources, *selected_index);
            }
            MenuState::ParameterConfig { mode_name, source_name, source_params, editing_param_index, param_names, .. } => {
                self.render_parameter_config(frame, inner, config, mode_name, source_name, source_params, editing_param_index, param_names);
            }
        }
    }
    
    fn render_mode_select(&self, frame: &mut ratatui::Frame, area: ratatui::prelude::Rect, config: &Config, modes: &[String], selected_index: usize) {
        let items: Vec<Line> = modes.iter().enumerate().map(|(i, mode)| {
        ...
        let list = List::new(items)
            .block(Block::default().title("Select Mode"));
            
        frame.render_widget(list, area);
    }
    
    fn render_source_select(&self, frame: &mut ratatui::Frame, area: ratatui::prelude::Rect, config: &Config, mode_name: &str, sources: &[String], selected_index: usize) {
        let items: Vec<Line> = sources.iter().enumerate().map(|(i, source)| {
            let style = if i == selected_index {
                Style::new().fg(config.theme.text.highlight).reversed()
            } else {
                Style::new()
    ...
    fn render_parameter_config(&self, frame: &mut ratatui::Frame, area: ratatui::prelude::Rect, config: &Config, mode_name: &str, source_name: &str, params: &ParameterValues, editing_index: &Option<usize>, param_names: &[String]) {
        let mut lines = vec![
        ...
        for (i, param_name) in param_names.iter().enumerate() {
            let value = params.get_as_string(param_name).unwrap_or_else(|| "<not set>".to_string());
            let style = if Some(i) == *editing_index {
                Style::new().fg(config.theme.text.highlight).reversed()
            } else {
                Style::new()
            };
            lines.push(Line::from(Span::styled(format!("{}: {}", param_name, value), style)));
        }
    ...
    pub fn handle_events(
        ...
        if let Event::Key(key) = event && key.is_press() {
            match &mut self.state {
                MenuState::ModeSelect { modes, selected_index } => {
                    match key.code {
                        KeyCode::Up => {
                            *selected_index = if *selected_index == 0 { modes.len() - 1 } else { *selected_index - 1 };
                        }
                        KeyCode::Down => {
                        ...
                        KeyCode::Enter => {
                            ...
                            if let Some(mode_config) = self.get_mode_config(&mode_name) {
                                ...
                                let allowed_sources = mode_config.allowed_sources.clone()
                ...
                MenuState::SourceSelect { mode_name, mode_params, available_sources, selected_index } => {
                    match key.code {
                        KeyCode::Up => {
                            *selected_index = if *selected_index == 0 { available_sources.len() - 1 } else { *selected_index - 1 };
                        }
                        KeyCode::Down => {
                        ...
                        KeyCode::Esc => {
                            let modes: Vec<String> = self.mode_configs.iter().map(|m| m.name.clone()).collect();
